...
void HdlLocalizationNodelet::onInit() {
  ...
  if (use_global_localization_) {
    ...
    set_global_map_service_ = nh_.serviceClient<hdl_global_localization::SetGlobalMap>("/hdl_global_localization/set_global_map");
    query_global_localization_service_ = nh_.serviceClient<hdl_global_localization::QueryGlobalLocalization>("/hdl_global_localization/query");
...
pclomp::NormalDistributionsTransform<HdlLocalizationNodelet::PointT, HdlLocalizationNodelet::PointT>::Ptr HdlLocalizationNodelet::createRegistration() const {
  ...
  if (reg_method == "NDT_OMP") {
    ...
    //   boost::shared_ptr<fast_gicp::NDTCuda<HdlLocalizationNodelet::PointT, HdlLocalizationNodelet::PointT>> ndt(new fast_gicp::NDTCuda<HdlLocalizationNodelet::PointT,
    //   HdlLocalizationNodelet::PointT>); ndt->setResolution(ndt_resolution);
...
void HdlLocalizationNodelet::pointsCallback(const sensor_msgs::PointCloud2ConstPtr& points_msg) {
  ...
  if (use_imu_) {
    ...
    if (tf_buffer_.canTransform(base_frame_, odom_stamp_last_, base_frame_, ros::Time(0), odom_frame_, ros::Duration(0.1))) {
      // Get the amount of odometry movement since the last calculation
      // Coordinate system where the front of the robot is x
      geometry_msgs::TransformStamped odom_delta = tf_buffer_.lookupTransform(base_frame_, odom_stamp_last_, base_frame_, ros::Time(0), odom_frame_);
...
void HdlLocalizationNodelet::publishOdometry(const ros::Time& stamp, const Eigen::Matrix4f& pose, double pose_covariance[36]) {
  ...
  if (publish_tf_) {
    if (tf_buffer_.canTransform(odom_frame_, base_frame_, ros::Time(0))) {
      ...
      geometry_msgs::TransformStamped frame_wrt_odom = tf_buffer_.lookupTransform(odom_frame_, base_frame_, ros::Time(0), ros::Duration(0.1));
 ...
 */
void HdlLocalizationNodelet::publishScanMatchingStatus(const std_msgs::Header& header, pcl::PointCloud<pcl::PointXYZI>::ConstPtr aligned) {
