...
PoseSystem::VectorX PoseSystem::computeNextState(const PoseSystem::VectorX& current_state) const
{
  PoseSystem::VectorX next_state(16);
...
PoseSystem::VectorX PoseSystem::computeNextStateWithIMU(const PoseSystem::VectorX& current_state,
...
PoseSystem::VectorX PoseSystem::computeNextStateWithOdom(const PoseSystem::VectorX& current_state,
                                                         ...
                                                         const Eigen::Vector3f& odom_twist_ang) const
{
  ...
  // Orientation
  Quaternion dq(1, raw_ang_vel[0] * time_step_ / 2.0, raw_ang_vel[1] * time_step_ / 2.0,
                raw_ang_vel[2] * time_step_ / 2.0);
  dq.normalize();
  Quaternion next_qt = (qt * dq).normalized();
  next_state.middleRows(6, 4) << next_qt.w(), next_qt.x(), next_qt.y(), next_qt.z();
  next_state.middleRows(10, 3) = current_state.middleRows(10, 3);  // Constant bias on acceleration
...
PoseSystem::VectorX PoseSystem::computeObservation(const PoseSystem::VectorX& current_state) const
{
  PoseSystem::VectorX observation(7);
