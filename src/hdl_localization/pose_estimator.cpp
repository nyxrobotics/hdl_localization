...
PoseEstimator::PoseEstimator(pclomp::NormalDistributionsTransform<PointT, PointT>::Ptr& registration,
                             ...
                             double cool_time_duration, double fitness_reject, double fitness_reliable,
                             double linear_correction_gain, double angular_correction_gain,
                             double angular_correction_distance_reject, double angular_correction_distance_reliable)
...
{
  ROS_WARN("Pamans: %f, %f, %f, %f, %f, %f, %f", cool_time_duration_, fitness_reject_, fitness_reliable_,
           linear_correction_gain_, angular_correction_gain_, angular_correction_distance_reject_,
                                                                   ...
                                                                   double pose_covariance[36])
{
  ...
  {
    ROS_WARN_THROTTLE(5.0, "Scan matching transformation probability is low (%f). Skip correction.",
                      transform_probability);
    return aligned;
  }
  double iter = registration_->getFinalNumIteration();
  // ROS_WARN_THROTTLE(1.0, "Scan matching fitness score: %f (near: %f, prob: %f, iter: %f)", fitness_score,
  // fitness_score_near, transform_probability, iter);
  ...
  {
    diff_angular_scaling =
        angular_correction_distance_reliable_ /
        (angular_correction_distance_reliable_ + diff_linear_norm / angular_correction_distance_reject_);
  }
  ...
  // ROS_WARN_THROTTLE(1.0, "fitness_scaling: %f, probability_scaling: %f, iter_scaling: %f", fitness_scaling,
  // probability_scaling, iter_scaling); When fitness_score is large, the gain of correction is reduced
  ...
  observation.middleRows(3, 4) =
      Eigen::Vector4f(q_measure_smooth.w(), q_measure_smooth.x(), q_measure_smooth.y(), q_measure_smooth.z());
